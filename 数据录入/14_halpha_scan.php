...
function filescanner(&$files,$path,$len,$year="",$month="",$day="",$level=0){
	...
	while(list($k,$v)=@each($paths)){
		...
		if(substr($v,0,1)!="."&&is_file($currpath)){ 
			...
			$time1="";
			if((int)$year>=2000) $time1=substr($v,4,2).":".substr($v,6,2).":".substr($v,8,2);
			...
			$time=$time." ".$time1;
			$name=$v;
			$sql="INSERT INTO  `bao`.`14_halpha_fit` (`file_id` ,`file_name` ,`file_path` ,`obsv_start_time` ,`obsv_end_time`)VALUES (NULL,  '$name',  '$path',  '$time',  '$time');";
			echo $sql."\n";
		}
	}
}
